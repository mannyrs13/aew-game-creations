...
# --- 3. PREMIUM AEW THEME CSS & SCROLLING FIX ---
st.markdown("""
    <style>
...
def place_talent(m_idx, s_idx):
    ...
    # Calculate score if match is full
        ...
        if st.session_state.current_draw == "CARD COMPLETE!":
            st.markdown("""
                <div class="clock-banner" style="border-color: #10b981;">
                    <span class="clock-text" style="color: #10b981;">🎉 CARD COMPLETE! ALL MATCHES BOOKED.</span>
                </div>
            """, unsafe_allow_html=True)
        else:
            st.markdown(f"""
    ...
    with st.container(border=True):
        ...
        st.markdown(f"""
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
                <h4 style="color: #ffd700; margin: 0; font-size: 1.1rem;">Match {m_idx + 1}: {match['type']}</h4>
                <span style="color: #10b981; font-weight: 800; font-size: 0.9rem;">{score_txt}</span>
            </div>
        """, unsafe_allow_html=True)
...
# --- 8. VIEW & COPY FINAL CARD EXPANDER ---
if st.session_state.card_data:
    st.markdown("<br>", unsafe_allow_html=True)
    with st.expander("📋 VIEW & COPY FINAL CARD", expanded=False):
        ...
        st.text_area("Select all and copy (Ctrl+C / Cmd+C):", value=full_plain_text, height=400)
